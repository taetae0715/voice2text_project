...
import csv
import os
from transformers import pipeline  # 요약을 위한 라이브러리 (예: T5, BART)
import torch
from pyannote.audio import Pipeline  # pyannote.audio로 화자 분리
...
def transcribe_and_save():
    ...
    try:
        ...
        # pyannote.audio로 화자 분리 (선택적 기능)
        pipeline = Pipeline.from_pretrained("pyannote/speaker-diarization")
        diarization = pipeline(audio_path)

        # 화자 분리 후 텍스트와 타임스탬프를 분리하여 정리
        segments = []
        current_speaker = None
        for speech_turn, _, speaker in diarization.itertracks(yield_label=True):
            segment_text = ""
            for segment in result["segments"]:
                if segment["start"] >= speech_turn.start and segment["end"] <= speech_turn.end:
                    segment_text += segment["text"] + " "
            segments.append((speaker, segment_text.strip()))

        # 요약 기능 (선택적)
        summarizer = pipeline("summarization")  # BART 또는 T5 모델을 사용한 요약
        ...
        for speaker, text in segments:
            text_output.insert(tk.END, f"Speaker {speaker}: {text}\n")
        
        # 요약 출력
        text_output.insert(tk.END, f"\n🎯 요약: {summary[0]['summary_text']}\n")
